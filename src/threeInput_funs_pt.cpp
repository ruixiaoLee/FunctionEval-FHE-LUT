...
int main() {
    ...
    size_t vSize = pow(2,2); // input table row size /* change the bit here! */
...
/* set all paramenters */
    ...
    vector<vector<int64_t>> vectorOfInLUT = read_table("Table/128bit/three/vectorOfInLUT_three_2.txt");/* change the bit here! */
    Plaintext plaintextIns = cryptoContext->MakePackedPlaintext(vectorOfInLUT[0]);
    vector<vector<int64_t>> vectorOfOuts = read_table("Table/128bit/three/vectorOfOutLUT_three_2.txt");/* change the bit here! */
